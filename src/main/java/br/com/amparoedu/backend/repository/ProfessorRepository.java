...
public class ProfessorRepository {
    ...
    // busca dados não sincronizados
    public List<Professor> naoSincronizados() {
    ...
    // marca os dados como sincronizado
    public void atualizarStatusSincronia(String id) {
        String sql = "UPDATE professores SET sincronizado = 1 WHERE id = ?";
        try (Connection conn = DatabaseConfig.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, id);
            stmt.executeUpdate();
